LkRosMap.views.geocoder.search = {
  html: " \
    <div id=\"LkRosMap.searchBox\">\
      <button id=\"LkRosMap.addressSearchButton\" class=\"lkrosmap-search-type-button lkrosmap-search-type-selected\" title=\"nach Adressen, Gemarkungen und Flurstücken suchen\"><i class=\"fa fa-home\"></i></button>\
      <button id=\"LkRosMap.featureSearchButton\" class=\"lkrosmap-search-type-button\" title=\"nach Objekten in der Karte suchen\"><i class=\"fa fa-map-marker\"></i></button>\
      <input id=\"LkRosMap.searchField\" type=\"text\" name=\"address\" size=\"50\" value=\"\" placeholder=\"Addresse, Gemarkung oder Flurstück eingeben\">\
      <i id=\"LkRosMap.searchFieldClear\" class=\"fa fa-times-circle lkrosmap-field-clearer\"></i>\
      <input type=\"button\" id=\"LkRosMap.searchLos\" value=\"Los\"/>\
      <div\
        id=\"LkRosMap.searchFieldResultBox\"\
        class=\"lkrosmap-search-result-box\"\
        style=\"display:none;\">\
      </div>\
    </div>\
  "
}